...
Les mots ou les chaînes de caractère forment les variables.
...
Les matrices en résultant sont potentiellement d’une très grande dimension (nombre de mots utilisés dans le corpus en colonnes), et en même temps creuses (les mots/chaînes de caractères employés dans le corpus peuvent être utilisés uniquement dans quelques documents du corpus).

Aussi, après l’importation d’un corpus de textes, la première étape consiste en une phase de pré-traitement visant à réduire la dimension de cette matrice : enlever le bruit(ponctuation, mots usuels n’apportant pas d’information, etc…), lemmatiser ou raciniser (ex : garder  exclusivement « jard » pour « jardin », « jardiner », « jardinage », …), faire une analyse en composantes principales…
