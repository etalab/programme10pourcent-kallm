...
Un système de RAG va combiner la capacité de génération de textes d'un LLM avec de la recherche d'information dans une base de connaissance interne. L'architecture classique d'un système de RAG se compose donc principalement de deux parties, une première partie de retrieval dont le rôle est d'analyser une question de l'utilisateur et de
...
Dans cette méthode on utilise un réseau de neurones appelé encodeur qui va transformer le texte en vecteurs de grandes dimensions que l'on stocke dans une base de données vectorielle. Cet encodeur a été entrainé a projeter des textes sémantiquement proches sur des vecteurs similaires, ce qui permet en comparant des vecteurs entre eux de trouver des textes de sens proche.
...
Ainsi lorsqu'une requête utilisateur est reçue en entrée, elle est elle aussi encodée dans un vecteur de même dimension par le réseau de neurones. On peut alors comparer ce vecteur à l'ensemble des
vecteurs présents en base ce qui permet de récupérer les vecteurs les plus proches qui correspondent à ceux qui sont sémantiquement proches. On peut ainsi trouver les chunks correspondant et les envoyer au LLM pour la génération.

### 3. Génération

La phase de génération dans un système de RAG intervient après la récupération des documents pertinents. Une fois les documents récupérés, ils sont insérés dans un prompt qui sera donné en entrée du LLM utilise pour qu'il produise des réponses contextuellement appropriées et précises. Plus le LLM est performant et correctement aligné sur les préférences humaines plus il est capable de prendre une quantité importante de document en contexte et ainsi de mieux répondre à la question.
...
L'architecture d'un système de RAG telle que présentée, est relativement simple et se révèle souvent insuffisante dans la réalité. En effet plusieurs difficultés peuvent apparaitre dans des cas d'usages réels :
 ...
 -  L'analyse des documents d'une base de connaissance peut s'avérer complexe à mettre en oeuvre du fait d'une grande diversité de formats et notamment pa présence de PDFs avec une structure compliquée. Cette grande hétérogénéité peut rendre complexe la création d'un système d'ingestion de données fiable. 

- La nécessité de mettre à jour régulièrement nécessite de créer un système de mise à jour de la base de donnée vectorielle

- La nature des documents peut entrainer le fait que les chunks ne sont pas suffisamment informatifs ou manquent de contexte pour être correctement compris par le LLM. 

- La partie de retrieval peut avoir des performances médiocres impactant toutes la suite du processus de RAG. 

Pour pallier ces difficultés, de nombreuses solutions techniques existent permettant de limiter l'étendue des problèmes précédemment soulevés. Premièrement l'amélioration du pipeline de chunking en enrichissant les chunks avec des informations contextuelles. Ces informations peuvent être le titre du document source par exemple, mais aussi un résumé du document, ou toute autre information pertinente pour améliorer la compréhension du chunk. 

### 5. Comparatif détaillé des solutions de stockages de données pour la recherche vectorielle approximative
